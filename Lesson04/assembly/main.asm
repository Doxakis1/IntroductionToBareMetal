...
.equ PINB , 0x03	; define the PORTB address as per ATmega328P docs
...
SETUP:
	...
	ldi	r16, 0x0	; set DDRB low
	out	DDRB, r16	; now we can read from PINB
	; we are going to sleep at the begining to wait for the whole system
	; to be ready
	rcall	SLEEP_FOR_500ms
	; then we are gonna get the initial PORTB5 state	
	in	r16, PINB
	bst	r16, 0x05	; store PORTB5
_wait_for_input:
	rcall	SLEEP_FOR_500ms	; sleep half a second
	in	r15, PINB	; get PORTB5
	bst	r15, 0x05	; get PORTB5 state
	cp	r15, r16	; compare previous state with new state
	brne	SET_YELLOW_PINS_ON; if change occured jump to the switching
	rjmp	_wait_for_input	; else sleep and wait again
SET_YELLOW_PINS_ON:
